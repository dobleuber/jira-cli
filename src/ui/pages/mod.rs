...
impl Page for HomePage {
    fn draw_page(&self) -> Result<()> {
        ...
        let database = self.db.database.read_db()?;
        let epics = database.epics;

        let mut epics: Vec<_> = epics.iter().collect();
        epics.sort_by_key(|&(key, _)| key);
        epics.iter().for_each(|&(k, epic)| {
            let id_string = get_column_string(&k.to_string(), 12);
            let epic_name = get_column_string(&epic.name, 34);
            let status = get_column_string(&epic.status.to_string(), 16);
            println!("{id_string}|{epic_name}|{status}");
        });
        println!();
        println!();
        ...
        match input {
            ...
            _ => match input.parse::<u32>() {
                Ok(epic_id) => {
                    ...
                    match database.epics.contains_key(&epic_id) {
                        true => Ok(Some(Action::NavigateToEpicDetail { epic_id })),
                        false => Ok(None),
                    }
                }
...
impl Page for EpicDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let epic_description = get_column_string(&epic.description, 28);
        let status = get_column_string(&epic.status.to_string(), 16);
        println!("{id}|{epic_name}|{epic_description}|{status}");
        ...
        let mut stories: Vec<_> = stories.iter().collect();
        stories.retain(|&(key, _)| epic.stories.contains(key));
        stories.sort_by_key(|&(key, _)| key);
        stories.iter().for_each(|&(k, story)| {
            let id_string = get_column_string(&k.to_string(), 12);
            let story_name = get_column_string(&story.name, 34);
            let status = get_column_string(&story.status.to_string(), 16);
            println!("{id_string}|{story_name}|{status}");
        });
    ...
    fn handle_input(&self, input: &str) -> Result<Option<Action>> {
        match input {
            ...
            _ => match input.parse::<u32>() {
                Ok(story_id) => {
                    let database = self.db.read_db()?;
                    match database.stories.contains_key(&story_id) {
...
impl Page for StoryDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let story_description = get_column_string(&story.description, 28);
        let status = get_column_string(&story.status.to_string(), 16);
        println!("{id}|{story_name}|{story_description}|{status}");
